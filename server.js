...
app.get('/api/weather-detail', async (req, res) => {
    ...
    try {
        ...
        // Fix: Fallback index jika jam tidak ketemu
...
// --- API Route Rute & AI (DIPERBAIKI UNTUK DATA 3 JAM) ---
app.get('/api/route-weather', async (req, res) => {
    ...
    try {
        ...
        // Jam yang diminta (Int)
        const startHour = startTime ? parseInt(startTime) : new Date().getHours();
        
        let rainPoints = [];
        let hourlyForecasts = []; // Array untuk 3 jam ke depan
        let shouldCarryUmbrella = false;
        let highestRain = 0;
        let startWeatherCode = 0; 

        // Loop menggunakan INDEX (j) agar kita bisa deteksi titik pertama dengan pasti
        for (let j = 0; j < checkPoints.length; j++) {
            const point = checkPoints[j];
            ...
            // FIX: Pencarian Jam yang Lebih Kuat (String Matching)
            // Mencari string "T08:00" misalnya, agar tidak bingung dengan konversi timezone
            const paddedHour = String(startHour).padStart(2, '0');
            let startIndex = hourlyData.time.findIndex(t => t.includes(`T${paddedHour}:00`));
            
            // Fallback: Jika tidak ketemu (beda hari), pakai index 0 biar tetap muncul data
            if (startIndex === -1) startIndex = 0;

            // Ambil data untuk titik ini
            ...
            // Simpan kode awal untuk Markov
            if (j === 0) startWeatherCode = wmoCode;
            ...
            // FIX: Ambil Data 3 Jam Kedepan KHUSUS untuk Titik Awal (j === 0)
            // Kita lakukan loop terpisah kecil di sini
            if (j === 0) {
                for (let k = 0; k < 3; k++) {
                    const idx = startIndex + k;
                    if (idx >= hourlyData.time.length) break;

                    const fCode = hourlyData.weather_code[idx];
                    const fRain = hourlyData.precipitation[idx];
                    const fTemp = hourlyData.temperature_2m[idx];
                    const fIsDay = hourlyData.is_day[idx] === 1;
                    const fTimeStr = hourlyData.time[idx];
                    const fHour = new Date(fTimeStr).getHours();
                    const fMap = mapWeatherCode(fCode, fIsDay);

                    hourlyForecasts.push({
                        hour: fHour,
                        condition: fMap.conditionText,
                        icon: fMap.iconClass,
                        temp: fTemp,
                        rain: fRain.toFixed(1)
                    });
                }
            }
        }
        ...
        const finalAdvice = {
            ...
            markovRiskScore: markovAnalysis.riskScore,
            forecast: hourlyForecasts, // Data ini yang tadi kosong, sekarang sudah diperbaiki
        };
...
app.get('/api/weather-news', async (req, res) => {
    try {
        ...
        res.json({ news: items });
    } catch (error) { res.status(500).json({ news: [], error: 'Gagal mengambil berita.' }); }
});
